...
import { AllTasksListItems, AllTasksForm } from '../../index';
...
const AllTasksList: React.FC<MapDispatchPropsType & ownProps> = React.memo(
  ({ list, setNewSidebarListName, getListsTasks }) => {
    React.useEffect(() => {
      getListsTasks();
    }, [getListsTasks]);
        ...
        {list && (
          <div className={cn(styles.todo__tasks_list)}>
            ...
            {list &&
              ...
              list.tasks.map((task) => (
                // @ts-ignore
                <AllTasksListItems key={task.id} task={task} />
              ))}
